...
void SubstituteByte(uint8_t *current_state){
    for(uint8_t i=0;i<16;i++){
        ...
        current_state[i] = Inverse_S_box[j>>4][j&0x0f];
    }
};

void InverseShiftRows(uint8_t *current_state){
    int k = 1;
    uint8_t temp[16];
    for(int i=0;i<16;i++)
...
void Inversecipher(uint8_t input[16], uint8_t output[16],uint8_t word[44][4]){
    uint8_t current_state[16];
    for(int i=0;i<16;i++){
        current_state[i] = input[i];
    }
    AddRoundKey(current_state,word,40);
    InverseShiftRows(current_state);
    SubstituteByte(current_state);
    for(int i=0;i<9;i++){
        AddRoundKey(current_state,word,36 - (4*i));
